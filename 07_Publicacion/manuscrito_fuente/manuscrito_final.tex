...
The most consequential adjustments narrowed overclaims. For RNF-EXP-01, the final wording removed a general reference to observations and retained only the objective/need plus relevant recorded constraints. For RNF-EXP-02, participants retained the need to show trainer review and professional provenance but avoided inventing states not observed in the evidence. For RNF-EXP-03, all three participants removed an automatic comparison between routine versions and kept an inspectable history with dates and available observations. For RNF-EXP-04, all three removed the claim that the system must compare alternatives and retained only the principal criterion behind a future suggestion or automatic assignment.
...
The second implication concerns the granularity of evidence. Technical walkthroughs contributed more explainability-relevant fragments (7 versus 2), but that asymmetry cannot be interpreted as a population effect. Technical participants also produced more coded fragments overall (49 versus 27). With only three sessions per profile and no quantitative participant-level outcome, an inferential group comparison would violate the data structure. The defensible finding is qualitative: technical sessions more often articulated provenance, validation, security, and system-oriented rationale, while non-technical evidence helped constrain clarity and avoid overcomplicated comparisons.
...
External & Three sessions per profile. & Descriptive profile comparison only. & Larger replication needed.\\
...
The preregistration is publicly identified at OSF (DOI: \href{https://doi.org/10.17605/OSF.IO/62YSC}{10.17605/OSF.IO/62YSC}). The final replication package is prepared for Zenodo but its persistent DOI must be inserted only after the external deposit is actually published; no Zenodo DOI is invented in this manuscript version. Public materials include anonymized transcripts, the anonymized questionnaire, requirement corpora, traceability, processed results, and reproducible scripts. Identifiable audio/video and original consent materials remain restricted and are not part of the open dataset. Data publication follows FAIR and software-citation principles \cite{wilkinson2016,smith2016,dicosmo2020}.
...
\textbf{Ethics and consent.} The project maintains public and restricted evidence zones. Statements about signed documents are limited to artifacts actually signed by the team; unsigned drafts are not represented as approved.\\
...
Table~\ref{tab:repro-map} links the main numerical claims used in the manuscript to the versioned input and the generated CSV that supports the claim. The entry point is \texttt{06\_Experimento/scripts\_analisis/run\_all.py}; the Phase 4 patch additionally exports the publication-facing copies of figures and tables into \texttt{07\_Publicacion/}. This appendix is an audit aid rather than a new analysis.
...
Mann--Whitney $U$, technical vs non-technical & Not applicable & Three WALK sessions per profile provide coded qualitative fragments but no preregistered independent quantitative participant outcome.\\
...
\midrule
RNF-EXP-C01 $\rightarrow$ RNF-EXP-01 & 2 & 1 & 0 & Retained with narrower wording on objective/need and relevant recorded constraints.\\
RNF-EXP-C02 $\rightarrow$ RNF-EXP-02 & 2 & 1 & 0 & Retained with trainer-review status and external-professional provenance when recorded.\\
RNF-EXP-C03 $\rightarrow$ RNF-EXP-03 & 0 & 3 & 0 & Retained after removing automatic comparison; history, dates and available observations remain.\\
RNF-EXP-C04 $\rightarrow$ RNF-EXP-04 & 0 & 3 & 0 & Retained after removing alternative comparison; only the principal suggestion/assignment criterion remains.\\
\bottomrule
\end{tabularx}
\end{table}
