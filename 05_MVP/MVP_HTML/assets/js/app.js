...
(() => {
  ...
  function renderDashboard(){
    ...
    $('#view').innerHTML=pageHead('Resumen operativo','Indicadores sintéticos del MVP final de FabroGym.')+`
      ...
      <div class="grid grid-half">
        <section class="card card-pad"><div class="section-title-row"><h3>Alertas de membresía</h3><span class="badge warning">RF-10</span></div><div class="mini-list">${state.clients.slice().sort((a,b)=>a.expiry.localeCompare(b.expiry)).slice(0,6).map(c=>`<div class="mini-row"><div><strong>${esc(c.name)}</strong><small>${esc(plan(c.planId)?.name||'Sin plan')} · vence ${esc(c.expiry)}</small></div><span class="badge ${isMembershipValid(c)?(c.expiry<=addDays(today(),7)?'warning':''):'danger'}">${isMembershipValid(c)?'Vigente':'No vigente'}</span></div>`).join('')}</div></section>
        <section class="card card-pad"><div class="section-title-row"><h3>Cobertura 2B</h3><span class="badge">C3</span></div><div style="text-align:center;padding:28px 10px"><div style="font-size:34px;font-weight:800;color:var(--green)">POR VERIFICAR</div><p class="muted">Cobertura Must sujeta a verificación terminal C3.</p><div class="progress"><span style="width:0%"></span></div></div></section>
      </div>`;
  }
  ...
  function renderClients(){
    $('#view').innerHTML=pageHead('Clientes y membresías','Registro, consulta, actualización, renovación y vigencia.')+`
      ...
      <aside class="card side-summary"><h3>RF cubiertos</h3><div class="summary-kv"><div class="row"><span>Registrar</span><b>RF-04</b></div><div class="row"><span>Consultar</span><b>RF-05</b></div><div class="row"><span>Actualizar/reactivar</span><b>RF-06</b></div><div class="row"><span>Renovar</span><b>RF-08</b></div><div class="row"><span>Vigencia</span><b>RF-09</b></div></div></aside></div>
  ...
  function renderPayments(){
    $('#view').innerHTML=pageHead('Pagos y comprobantes','RF-11 · Registro interno demostrativo, sin facturación electrónica ni pasarela bancaria.')+`
  ...
  function renderCoverage(){
    const coverage=[
      ['RF-01','Autenticar usuario'],['RF-02','Aplicar permisos por rol'],['RF-04','Registrar cliente mínimo'],['RF-05','Buscar y consultar cliente'],['RF-06','Actualizar/reactivar cliente'],['RF-07','Configurar planes y promociones'],['RF-08','Activar o renovar membresía'],['RF-09','Consultar vigencia'],['RF-10','Alertar vencimientos'],['RF-11','Registrar pago y comprobante'],['RF-13','Registrar asistencia'],['RF-14','Consultar asistencia'],['RF-15','Administrar productos'],['RF-20','Gestionar novedades internas'],['RF-22','Administrar rutinas'],['RF-23','Registrar seguimiento']
    ];
    $('#view').innerHTML=pageHead('Cobertura de requisitos Must','Entrega 4 (2B) · Criterio C3 · cobertura mínima requerida: 80 %.')+`<div class="grid grid-half"><section class="card card-pad"><div style="text-align:center;padding:26px"><div style="font-size:42px;font-weight:800;color:var(--green)">POR VERIFICAR</div><h3 style="margin:6px 0">19 RF Must</h3><p class="muted">Las fuentes históricas divergen entre 12/19 y 16/19; C3 debe verificar código y pruebas.</p><div class="progress" style="height:12px"><span style="width:0%"></span></div></div></section><section class="card card-pad"><h3 class="section-title">Estado de este bloque</h3><div class="tip-box"><strong>Sin inferencia</strong>La normalización de IDs no acredita implementación ni resuelve la cobertura del MVP.</div></section></div><section class="card" style="margin-top:20px"><div class="table-wrap"><table class="table"><thead><tr><th>ID</th><th>Requisito</th><th>Estado</th></tr></thead><tbody>${coverage.map(([id,d])=>`<tr><td><strong>${id}</strong></td><td>${esc(d)}</td><td><span class="badge gray">Por verificar C3</span></td></tr>`).join('')}</tbody></table></div></section>`;
  }
